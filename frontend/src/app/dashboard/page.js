  ...
  return (
    <div>
      ...
      <div className="stat-cards-grid">
        {[
          { icon: <WeatherAnimatedIcon temperature={analytics?.latest_weather?.temperature || 29} size={44} />, label: t('current_temp'), value: `${Math.round(analytics?.latest_weather?.temperature || 29)}°C`, sub: lang === 'tl' ? '↑ +0.8° vs kahapon' : '↑ +0.8° vs yesterday', color: 'green', subColor: 'var(--success)' },
          { icon: <WeatherAnimatedIcon condition="rain" size={44} />, label: t('rainfall_30d'), value: `${Math.round(analytics?.summary?.total_rainfall_30d_mm || 0)}mm`, sub: lang === 'tl' ? '↑ Mas mataas sa normal' : '↑ Above average', color: 'amber', subColor: 'var(--success)' },
          ...
          <div key={s.label} className={`stat-card ${s.color}`}>
            ...
            <div className={`stat-icon ${s.color}`}>
              {typeof s.icon === 'string' ? <span style={{ fontSize: 26 }}>{s.icon}</span> : s.icon}
            </div>
